  ...
  return (
    <div className="min-h-screen bg-[#03040a] text-white pt-40 pb-24 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        <div className="mb-16">
          <Link href="#" className="group relative block overflow-hidden rounded-3xl border border-white/10 bg-white/[0.02] transition-all hover:border-white/20">
            <div className="grid md:grid-cols-2">
              ...
              <div className="flex flex-col justify-center p-8 md:p-12">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <span className="text-[10px] uppercase tracking-widest text-white/40">{news[0].readTime} de lecture</span>
                </div>
        ...
        <div className="grid gap-8 md:grid-cols-2">
          {news.slice(1).map((item) => (
            <Link key={item.title} href="#" className="group flex flex-col rounded-3xl border border-white/10 bg-white/[0.02] overflow-hidden transition-all hover:border-white/20">
              ...
              <div className="flex flex-1 flex-col p-8">
                <div className="mb-4 text-[10px] uppercase tracking-widest text-white/40">
                  {new Date(item.date).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' })} — {item.readTime}
                </div>
        ...
        <div className="mt-24 rounded-3xl border border-white/5 bg-white/[0.01] p-12 text-center">
          ...
          <p className="text-white/50 mb-10 max-w-xl mx-auto">
            Suivez-nous également sur nos réseaux sociaux pour les résultats en direct et les coulisses du club.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
             {['Instagram', 'Facebook', 'LinkedIn'].map(social => (
               <button key={social} className="rounded-full border border-white/10 bg-white/5 px-8 py-3 text-xs font-bold uppercase tracking-widest transition-all hover:bg-white hover:text-black">
                 {social}
               </button>
             ))}
          </div>
