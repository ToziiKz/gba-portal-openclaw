...
export const metadata: Metadata = {
  ...
  description: "Comprendre comment GBA Portal traite les données : finalités, sécurité, droits et contact.",
  alternates: {
    canonical: "/privacy",
  },
};
    ...
    >
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-3xl border border-white/10 bg-black/40 p-6">
            ...
            <p className="mt-3 text-sm text-white/70">
              Email, rôle (staff/parent/partenaire), et informations nécessaires à l’activation d’accès.
            </p>
          </div>
          <div className="rounded-3xl border border-white/10 bg-black/40 p-6">
            <p className="text-xs uppercase tracking-[0.6em] text-white/50">Usage produit</p>
            <p className="mt-3 text-sm text-white/70">
              Éléments techniques (logs, erreurs, sécurité) pour fiabiliser l’expérience et prévenir les abus.
            </p>
          </div>
        </div>
        <p className="text-xs text-white/45">
          Le contenu exact dépend des fonctionnalités activées (news, inscriptions, sponsoring). Objectif : du signal utile, pas de curiosité.
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm leading-relaxed text-white/70">
          Le site peut utiliser des cookies (ou mécanismes équivalents) nécessaires au fonctionnement : session, sécurité, préférences.
          Certaines animations peuvent utiliser le stockage local (ex. ne pas rejouer un effet visuel à chaque visite).
        </p>
        <p className="text-sm leading-relaxed text-white/70">
          Si des cookies non essentiels sont ajoutés (mesure d’audience, marketing), ils doivent être documentés et soumis à consentement.
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm leading-relaxed text-white/70">
          Vous pouvez demander l’accès, la rectification ou la suppression de vos données, dans les limites légales et opérationnelles.
          Contact : <a className="underline decoration-white/30 underline-offset-4 hover:text-white" href={`mailto:${contactEmail}`}>{contactEmail}</a>.
        </p>
        <p className="text-xs text-white/45">
          Note : ce document est une base produit. Pour une conformité complète (RGPD), adaptez-le au contexte réel d’hébergement et d’usage.
        </p>
      </div>
    </TrustPageShell>
  );
