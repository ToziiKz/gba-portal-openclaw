...
function computeFormatFromTeamCategory(teamCategory: string | null | undefined): GameFormat {
        ...
        >
          <button
            ...
            onClick={(e) => {
              ...
              const raw = window.prompt('Numéro du joueur (1-99) :', String(slot.number))
              if (!raw) return
              const n = Number(raw)
...
export default function TactiquePage() {
  ...
  const availableFormations = React.useMemo(() => {
    ...
    if (format === 8) return ['3-3-1', '2-4-1', '3-1-3', '2-3-2', '3-2-2', '2-3-1-1']
  ...
  const handleFormationChange = (nextFormation: string) => {
    ...
    setSlots((prev) => {
      const currentPlayers = prev.map((s) => s.playerId).filter(Boolean) as string[]
  ...
  const exportImage = async () => {
    ...
    const dataUrl = await toPng(pitchRef.current, { cacheBust: true, pixelRatio: 3 })
  ...
  const { setNodeRef: benchRef, isOver: benchOver } = useDroppable({ id: 'bench' })
          ...
          <Card className="premium-card card-shell rounded-3xl">
            ...
            <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4">
              ...
              {isLoading ? (
                ...
                <p className="py-6 text-center text-sm text-white/40">Tous les joueurs sont placés.</p>
              ) : (
                <div
                  ref={benchRef}
                  className={
                    `mt-4 grid grid-cols-2 gap-2 rounded-xl p-2 transition ` +
                    (benchOver ? 'bg-emerald-500/10 border border-emerald-500/30' : '')
                  }
                >
                  {bench.map((p) => (
            ...
            <p className="mt-4 text-xs text-white/40">
              Astuce: glisse un joueur sur un poste. Glisse sur le banc pour retirer.
            </p>
            <p className="mt-2 text-[11px] text-white/35 font-mono break-all">{debug}</p>
          </CardContent>
        </Card>
      </div>
            ...
            {slots.map((s) => {
              ...
              return (
                <SlotView
                  ...
                  slot={s}
                  playerName={name}
                  onClear={() => setSlots((prev) => prev.map((x) => (x.id === s.id ? { ...x, playerId: null } : x)))}
                  onSetNumber={(n) =>
                    setSlots((prev) => prev.map((x) => (x.id === s.id ? { ...x, number: n } : x)))
                  }
                />
              )
            })}
