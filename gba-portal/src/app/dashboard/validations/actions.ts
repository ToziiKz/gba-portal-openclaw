...
const handlers: Record<string, (sb: SupabaseClient, p: Payload) => Promise<{ error: { message: string; code?: string } | null }>> = {
  ...
  'players.delete': async (sb, p) => {
    return sb.from('players').delete().eq('id', p.id as string)
  },
