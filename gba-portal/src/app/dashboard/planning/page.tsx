    ...
    <div className="grid gap-6">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-white/70">
          Vue semaine + filtres. Cliquez sur un créneau pour voir les détails ou gérer les présences.
        </p>
      </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <p className="mt-1 text-sm text-white/65">Essayez de changer de pôle ou d’élargir la recherche.</p>
            </div>
          ) : (
            <div className="grid gap-3 md:grid-cols-3 lg:grid-cols-7">
              {planningDays.map((day) => {
                ...
                return (
                  <section key={day} aria-label={`Jour ${day}`} className="rounded-2xl border border-white/10 bg-white/5 p-3">
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  <p className="truncate text-sm font-semibold text-white">{s.team}</p>
                              ...
                              <div className="mt-1 flex flex-wrap gap-1">
                                {s.staff.slice(0, 2).map((st) => (
                                  <span key={st} className="text-[10px] text-white/45 bg-white/5 px-1 rounded">
                                    {st}
                                  </span>
                                ))}
                                {s.staff.length > 2 && <span className="text-[10px] text-white/45">+{s.staff.length - 2}</span>}
                              </div>
                            </button>
                          </li>
                        ))}
          ...
          view === "presence" ? (
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300">
              ...
              <div className="pt-4 border-t border-white/10 flex justify-end gap-3">
                ...
                <Button onClick={() => setView("presence")}>
                  Gérer les présences
                </Button>
              </div>
      ...
      <p className="text-xs text-white/45">
        Note : mock data + state local (localStorage). L’ajout de séance est fonctionnel (client-side).
      </p>
    </div>
  );
}
