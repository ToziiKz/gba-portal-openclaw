...
async function getAuthContext() {
  ...
  return { supabase, user, role: String(profile?.role ?? 'viewer') }
}
...
export async function requestMovePlayer(formData: FormData) {
  ...
  const payload = { id: playerId, from_team_id: fromTeamId, team_id: toTeamId }
...
export async function requestDeletePlayer(formData: FormData) {
  ...
  const payload = { id: playerId, team_id: teamId }
