...
export default function DashboardPresencesPage() {
  ...
  React.useEffect(() => {
    ...
    if (poleRaw && poles.includes(poleRaw as AttendanceSession['pole']))
      setPole(poleRaw as AttendanceSession['pole'])
    ...
    <div className="grid gap-6">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm text-white/70">
          Suivi des présences par séance (mock) : sélection séance → liste joueurs → statut
          (présent/retard/justifié/absent). Données mock + state local uniquement.
        </p>
      </div>
      ...
      <Card className="premium-card card-shell rounded-3xl">
        <CardHeader>
          ...
          <CardDescription>
            Filtrer par pôle et rechercher une séance (équipe / lieu / jour).
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
            <p className="text-sm text-white/60" aria-live="polite">
              {isLoading
                ...
                : `${filteredSessions.length} séance(s) (mock)`}
            </p>
              ...
              <div className="grid gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-white/45">
                    maj {selectedSession.updatedAtLabel} (mock)
                  </p>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-xs text-white/45">
                    Actions locales — persistance navigateur (localStorage).
                  </p>
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() =>
                      dispatch({ type: 'resetSession', sessionId: selectedSession.id })
                    }
                  >
                  ...
                  <ul className="mt-3 grid gap-2">
                    {selectedSession.rows.map((row) => {
                      ...
                      const updatedAtLabel =
                        rowState?.updatedAtLabel ?? selectedSession.updatedAtLabel
                ...
                <p className="text-xs text-white/45">
                  Next : vue par joueur (historique), export feuille de match, liens planning ↔
