                ...
                <div className="flex justify-between items-start">
                    ...
                    {canWrite ? (
                      <div className="flex gap-2">
                        <Button size="sm" variant="ghost" onClick={() => handleEdit(selectedCategory)}>
                          Modifier
                        </Button>
                        <Button
                          size="sm"
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                ...
                <input name="name" defaultValue={currentCategory.name} required className="mt-1 w-full rounded border p-2 text-black" placeholder="ex: U10" />
            </div>
            <div>
                <label className="block text-sm font-medium text-black">Pôle</label>
                <select name="pole" defaultValue={currentCategory.pole || POLES[0]} className="mt-1 w-full rounded border p-2 text-black">
                    {POLES.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
            </div>
            <div>
                <label className="block text-sm font-medium text-black">Années (label)</label>
                <input name="age_range_label" defaultValue={currentCategory.age_range_label} className="mt-1 w-full rounded border p-2 text-black" placeholder="ex: 2013-2014" />
            </div>
            <div>
                <label className="block text-sm font-medium text-black">Équipes (label)</label>
                <input name="teams_label" defaultValue={currentCategory.teams_label} className="mt-1 w-full rounded border p-2 text-black" placeholder="ex: U10, U11" />
            </div>
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-sm font-medium text-black">Nb Équipes</label>
                    <input name="teams_count" type="number" defaultValue={currentCategory.teams_count || 0} className="mt-1 w-full rounded border p-2 text-black" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-black">Nb Joueurs (est.)</label>
                    <input name="players_estimate" type="number" defaultValue={currentCategory.players_estimate || 0} className="mt-1 w-full rounded border p-2 text-black" />
                </div>
            </div>
            <div>
                <label className="block text-sm font-medium text-black">Notes</label>
                <textarea name="notes" defaultValue={currentCategory.notes} className="mt-1 w-full rounded border p-2 text-black" />
            </div>
