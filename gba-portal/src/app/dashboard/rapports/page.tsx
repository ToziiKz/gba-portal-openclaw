  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <Card className="premium-card card-shell rounded-3xl">
          ...
          <CardContent>
            ...
            <p className="mt-3 text-sm text-white/65">Équipes : <span className="font-semibold text-white">{teamsCount ?? '—'}</span></p>
            <div className="mt-4 flex flex-wrap gap-2">
              <Link href="/dashboard/joueurs"><Button size="sm" variant="secondary">Ouvrir Joueurs</Button></Link>
              <Link href="/dashboard/equipes"><Button size="sm" variant="ghost">Ouvrir Équipes</Button></Link>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="premium-card card-shell rounded-3xl">
          ...
          <CardContent>
            <p className="text-3xl font-black tracking-tight text-white">{missingAttendance.length}</p>
            <p className="mt-1 text-xs text-white/50">séance(s) sans saisie (sur les prochaines)</p>
            <div className="mt-4">
              <Link href="/dashboard/presences"><Button size="sm">Aller aux Présences</Button></Link>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="premium-card card-shell rounded-3xl">
          ...
          <CardContent>
            <div className="grid gap-2">
              <Link href="/dashboard/planning"><Button variant="secondary" className="w-full">Planning club</Button></Link>
              <Link href="/dashboard/convocations"><Button variant="secondary" className="w-full">Convocations</Button></Link>
              <Link href="/dashboard/tactique"><Button variant="secondary" className="w-full">Tactique</Button></Link>
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          {nextSessions.length === 0 ? (
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
              ...
              <p className="mt-1 text-sm text-white/65">Ajoute des séances dans le planning pour démarrer.</p>
            </div>
          ) : (
                ...
                return (
                  <li key={s.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`shrink-0 rounded-full border px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.25em] ${hasAttendance ? 'border-green-400/20 bg-green-400/10 text-green-300' : 'border-yellow-400/20 bg-yellow-400/10 text-yellow-300'}`}>
                        {hasAttendance ? 'OK' : 'À faire'}
                      </span>
                    </div>

                    <div className="mt-3 flex flex-wrap gap-2">
                      <Link href="/dashboard/presences"><Button size="sm">Ouvrir Présences</Button></Link>
                      <Link href="/dashboard/planning"><Button size="sm" variant="ghost">Voir planning</Button></Link>
                    </div>
                  </li>
                )
              })}
      ...
      <p className="text-xs text-white/45">
        Note : cette page remplace l’ancien module “Rapports” (trop complexe). On garde uniquement du concret et actionnable.
      </p>
    </div>
  )
}
