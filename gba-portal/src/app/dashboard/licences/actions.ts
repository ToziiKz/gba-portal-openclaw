...
function deriveStatus(amountPaid: number, amountTotal: number): 'unpaid' | 'partial' | 'paid' {
...
export async function getLicences() {
  ...
  return (data || []).map((item: {
    ...
    const isOverdue = item.due_date ? new Date(item.due_date) < new Date() && item.amount_paid_eur < item.amount_total_eur : false
    return {
      id: item.id,
