...
import { registerLicencePayment, resetLicencePayment, type LicenceRow } from './actions'
...
export function LicencesClient({ initialRows }: { initialRows: LicenceRow[] }) {
  ...
  const handlePaymentSubmit = (
    ...
    startTransition(async () => {
      const result = await registerLicencePayment(openRowId, amountEur, method, note)
      if (result.error) {
        setToast(`Erreur: ${result.error}`)
      } else {
        setToast(sendReceipt ? 'Paiement enregistré + reçu envoyé (simulé)' : 'Paiement enregistré')
      }
      setOpenRowId(null)
    })
  }
