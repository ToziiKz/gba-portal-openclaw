...
export function useEquipementsState() {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(() => getPersistedRequests()[0]?.id ?? null)
  ...
  const filtered = useMemo(() => {
    ...
    return requests
      ...
      .filter((request) => {
        ...
        const haystack = `${request.title} ${request.team} ${request.category} ${request.priority}`.toLowerCase()
