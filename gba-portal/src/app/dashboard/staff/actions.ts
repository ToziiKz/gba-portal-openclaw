...
export async function updateStaffAvailability(id: string, availability: 'ok' | 'limited' | 'off') {
...
export async function updateStaffDetails(id: string, data: { email?: string; phone?: string; note?: string }) {
  const scope = await getDashboardScope()
  if (scope.role !== 'admin') {
