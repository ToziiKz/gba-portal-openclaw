...
export async function activateCoachAccount(_prevState: unknown, formData: FormData) {
  ...
  if (password.length < 8) {
    return { ok: false as const, error: 'Le mot de passe doit contenir au moins 8 caractères.' }
  }
  ...
  if (new Date(inv.expires_at).getTime() < Date.now()) {
    return { ok: false as const, error: 'Lien expiré. Demandez une nouvelle invitation.' }
  }
