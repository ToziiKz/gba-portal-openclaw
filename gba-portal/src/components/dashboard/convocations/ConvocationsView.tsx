      ...
      <Card className="premium-card card-shell rounded-3xl">
        ...
        <CardContent className="grid gap-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="grid gap-2">
              <span className="text-xs font-semibold uppercase tracking-[0.25em] text-white/60">Adversaire</span>
              <input value={opponent} onChange={(e) => setOpponent(e.target.value)} className={inputBaseClassName()} placeholder="Ex: FC Strasbourg" />
            </label>
            <label className="grid gap-2">
              <span className="text-xs font-semibold uppercase tracking-[0.25em] text-white/60">Date / heure</span>
              <input value={datetime} onChange={(e) => setDatetime(e.target.value)} className={inputBaseClassName()} placeholder="Ex: Sam 16:00" />
            </label>
          </div>

          <label className="grid gap-2">
            <span className="text-xs font-semibold uppercase tracking-[0.25em] text-white/60">Lieu</span>
            <input value={location} onChange={(e) => setLocation(e.target.value)} className={inputBaseClassName()} placeholder="Ex: Stade municipal" />
          </label>

          <label className="grid gap-2">
            <span className="text-xs font-semibold uppercase tracking-[0.25em] text-white/60">Note</span>
            <textarea value={note} onChange={(e) => setNote(e.target.value)} className={inputBaseClassName()} rows={4} placeholder="Ex: RDV 45 min avant, tenue complète…" />
          </label>

          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.25em] text-white/60">Joueurs</p>
            <p className="mt-2 text-sm text-white/70">
              {loading ? 'Chargement…' : `${selected.size} sélectionné(s) / ${players.length}`}
            </p>
            ...
            <div className="mt-3 max-h-[380px] overflow-y-auto pr-1 custom-scrollbar space-y-2">
              ...
              {players.length === 0 && !loading ? (
                <p className="text-sm text-white/50">Aucun joueur trouvé pour cette équipe.</p>
              ) : null}
            </div>
          </div>
        </CardContent>
      ...
      <Card className="premium-card card-shell rounded-3xl">
        ...
        <CardContent className="space-y-4">
          <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
            <pre className="whitespace-pre-wrap text-sm text-white/80 font-medium">{message}</pre>
          </div>
