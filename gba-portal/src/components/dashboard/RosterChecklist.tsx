...
import { dashboardPlayersMock, type DashboardPlayer } from "@/lib/mocks/dashboardPlayers";
...
const statusOptions: { value: AttendanceStatus; label: string; color: string }[] = [
  { value: "present", label: "Présent", color: "text-green-400 bg-green-400/10 border-green-400/20" },
  { value: "late", label: "Retard", color: "text-yellow-400 bg-yellow-400/10 border-yellow-400/20" },
...
export function RosterChecklist({ planningSessionId, team, onBack, onClose }: RosterChecklistProps) {
  ...
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className="space-y-2 max-h-[400px] overflow-y-auto pr-1 custom-scrollbar">
        ...
        {rows.length === 0 && (
            <p className="text-center text-sm text-white/40 py-4">Aucun joueur trouvé pour cette équipe.</p>
        )}
      </div>
