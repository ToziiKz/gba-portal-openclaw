                ...
                <div className="grid gap-4">
                  ...
                  <div>
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="text-xs uppercase tracking-[0.35em] text-white/55">Identité</p>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-3">
                      <p className="text-xs uppercase tracking-[0.35em] text-white/55 mb-2">Sportif</p>
                      
                      <div className="space-y-1">
                        <p className="text-[10px] uppercase text-white/40">Club</p>
                        <p className="text-sm font-semibold text-white">{selectedPlayer.club_name}</p>
                      </div>

                      <div className="space-y-1">
                        <p className="text-[10px] uppercase text-white/40">Licence</p>
                        <p className="text-sm font-mono text-white/80">{selectedPlayer.license_number ?? '—'}</p>
                      </div>
                      ...
                      {selectedPlayer.status_label && (
                        <div className="space-y-1 pt-2 border-t border-white/10 mt-2">
                          ...
                          <p className="text-xs text-white/70">
                            Du {selectedPlayer.status_start_date ?? '?'} au {selectedPlayer.status_end_date ?? '?'}
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-3">
                      <p className="text-xs uppercase tracking-[0.35em] text-white/55 mb-2">Contact</p>
                      
                      <div className="space-y-1">
                        <p className="text-[10px] uppercase text-white/40">Email</p>
                        <p className="text-sm text-white break-all">{selectedPlayer.email || '—'}</p>
                      </div>
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-3">
                    <p className="text-xs uppercase tracking-[0.35em] text-white/55 mb-2">Responsable Légal</p>
                    
                    <div className="grid sm:grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <p className="text-[10px] uppercase text-white/40">Nom</p>
                        <p className="text-sm font-semibold text-white">{selectedPlayer.legal_guardian_name || '—'}</p>
                      </div>
                      
                      <div className="space-y-1">
                        <p className="text-[10px] uppercase text-white/40">Adresse</p>
                        <p className="text-sm text-white/80">
                          {selectedPlayer.address_street}<br/>
                    ...
                    <div className="grid gap-4">
                      <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                        ...
                        <p className="mt-2 text-sm text-white break-all">{selectedPlayer.email || '—'}</p>
                    ...
                    <div className="grid gap-4">
                      <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                        <p className="text-xs uppercase tracking-[0.35em] text-white/55">Responsable légal</p>
                        <p className="mt-2 text-sm font-semibold text-white">{selectedPlayer.legal_guardian_name || '—'}</p>
                      </div>
                      <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                      <p className="text-xs uppercase tracking-[0.35em] text-white/55">Statistiques</p>
                      {presenceLoading ? (
                        ...
                        <div className="mt-4 grid gap-2">
                          <div className="flex justify-between text-sm text-white">
                            <span>Présences</span>
                            <span className="font-semibold text-green-400">{presenceStats.present}</span>
                          </div>
                          <div className="flex justify-between text-sm text-white">
                            <span>Retards</span>
                            <span className="font-semibold text-yellow-400">{presenceStats.late}</span>
                          </div>
                          <div className="flex justify-between text-sm text-white">
                            <span>Excusés</span>
                            <span className="font-semibold text-blue-400">{presenceStats.excused}</span>
                          </div>
                          <div className="flex justify-between text-sm text-white">
                            <span>Absences</span>
                            <span className="font-semibold text-red-400">{presenceStats.absent}</span>
                          </div>
