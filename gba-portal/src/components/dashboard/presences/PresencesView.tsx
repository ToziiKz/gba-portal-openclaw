    ...
    <div className="grid gap-6">
        ...
        <CardContent>
          <div className="grid gap-3 md:grid-cols-3">
            <label className="grid gap-1.5">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Équipe</span>
            ...
            <label className="grid gap-1.5 md:col-span-2">
              <span className="text-[11px] font-bold uppercase tracking-wider text-slate-500">Recherche</span>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap gap-2 text-[11px] font-bold uppercase tracking-wider">
              <span className="rounded-full bg-slate-100 px-3 py-1 text-slate-600">{filtered.length} séance(s)</span>
              {scope.role === 'coach' ? (
      ...
      <Card className="rounded-3xl border-slate-100 bg-white">
        <CardHeader>
          ...
          <CardDescription>Cliquer sur une séance pour ouvrir la feuille de présence.</CardDescription>
        </CardHeader>
        <CardContent>
          {filtered.length === 0 ? (
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">
                      ...
                      <ul className="mt-3 grid gap-2">
                        {items.map((s) => (
                          <li key={s.id}>
                            ...
                            >
                              <p className="truncate text-sm font-semibold text-slate-900">{s.team?.name ?? '—'}</p>
                              <p className="mt-1 text-[11px] text-slate-500">{s.pole}</p>
                              <p className="mt-1 text-[11px] text-slate-600">{sessionTimeLabel(s)}</p>
                            </button>
                          </li>
                        ))}
                      </ul>
